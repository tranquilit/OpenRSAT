...
uses
  ...
  Buttons,
  ExtCtrls,
  ButtonPanel, ActnList,
  uldapconfigs, mormot.core.base;
...
begin
  try
    ...
    if not LdapTest.Connect() then
      MessageDlg('Failure', GetLdapErrorCustomMessage(LdapTest), mtError, [mbOK], 0)
    else
      MessageDlg('Success', FormatUtf8('Authentication with user "%" has succeeded.', [LdapTest.Settings.UserName]), mtConfirmation, [mbOK], 0);
  finally
    FreeAndNil(LdapTest);
...
begin
  try
    ...
    if not LdapTest.Connect() then
      MessageDlg(rsLdapError, GetLdapErrorCustomMessage(LdapTest), mtError, [mbOK], 0)
    else
      MessageDlg(rsLdapSuccess, FormatUtf8('Connection to domain "%" with domain controller "%" on port "%" can be established.', [LdapTest.Settings.KerberosDN, LdapTest.Settings.TargetHost, LdapTest.Settings.TargetPort]), mtConfirmation, [mbOK], 0);
  finally
    FreeAndNil(LdapTest);
    FreeAndNil(ASettings);
  end;
...
begin
  ...
  //ComboBox_Algorithm.ItemIndex := ASettings.; ?
